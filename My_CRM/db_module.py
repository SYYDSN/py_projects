...
"""配置数据库相关参数"""
sql_host = "39.108.67.178"  # 生产用数据库地址
user = "root"  # 生产用数据库账户
password = "Ali@170525"  # 生产用数据库密码
database = "my_crm"  # 数据库名称
charset = "utf8"  # 数据库字符集
pool_size = 20  # 数据库连接池
...
# sql_host = "127.0.0.1"  # 调试用数据库地址
# password = "123456"  # 调试用数据库密码


def get_conn(host='127.0.0.1', a_user=user, a_password=password, a_database=database, a_charset=charset):
    """返回mysql数据库连接"""
    flag = True
    while flag:
        try:
            conn = pymysql.connect(host, a_user, a_password, a_database, charset=a_charset)
            flag = False
        except:
            print("数据库连接失败，请等待重试...")
            flag = True
...
"""用于备份的的engine"""
bak_engine = create_engine(
    "mysql+pymysql://{}:{}@localhost/my_crm_bak?charset=utf8".format(user, password),
    echo=False, echo_pool=False, max_overflow=2, pool_size=2, pool_recycle=pool_recycle,
    strategy='threadlocal')
...
"""用于备份的的engine
bak_engine = create_engine(
    "mysql+pymysql://admin_bak:admin_bak@localhost/my_crm_bak?charset=utf8",
    echo=False, echo_pool=False, max_overflow=2, pool_size=2, pool_recycle=pool_recycle,
    strategy='threadlocal')
"""

"""用于备份的Session"""
Bak_DB_Session = sessionmaker(bind=bak_engine, autoflush=False, autocommit=False)


def bak_sql_session():
    """返回一个SQLalchemy.orm.session.Session 的实例。此实例为备份专用"""
    return Bak_DB_Session()


# 没有mongodb
# MongoDB_Client = pymongo.MongoClient("mongodb://eai:Eai-1518@{}:27017/ocr".format(sql_host))
