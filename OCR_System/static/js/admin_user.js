...
$(function () {
    ...
    up_user = function ($obj) {
        ...
        $.post("/manage_users", {"sn": user_id, "the_type": "up"}, function (data) {
    ...
    down_user = function ($obj) {
        var user_id = $obj.attr("data-id");
        $.post("/manage_users", {"sn": user_id, "the_type": "down"}, function (data) {
            var data = JSON.parse(data);
            if (data['message'] == 'success') {
                alert("禁用成功");
                location.href = location.href;
    ...
    drop_user = function ($obj) {
        ...
        if (r) {
            var user_id = $obj.attr("data-id");
            $.post("/manage_users", {"sn": user_id, "the_type": "delete"}, function (data) {
                var data = JSON.parse(data);
                if (data['message'] == 'success') {
                    alert("删除成功");
                    location.href = location.href;
    ...
    $("#save_user").click(function () {
        ...
        if ($.trim($("#myModalLabel").text()) == "添加用户") {
            the_type = "add";
        ...
        $.post("/manage_users", args, function (message) {
    ...
    show_sftp = function(sn){
        ...
        "group_sn": sn, "user_class": 1};
        $.post("/manage_users", args, function(data){
            var data = JSON.parse(data);
            if(data['message'] != "success"){
                alert(data['message']);
                return false;
