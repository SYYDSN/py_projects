...
let custom_maker_hover = function(custom_marker_obj){
    ...
    AMap.event.addListener(custom_marker_obj, "mouseover", function(e) {
        ...
        label.parents(".amap-marker-label:first").animate({"width":0},80);
...
// 为每个自定义标记点增加鼠标点击事件
let custom_maker_double_click = function(custom_marker_obj){
    AMap.event.addListener(custom_marker_obj, "click", function(e){
        let user_id = e.target.G.extData;
        let loc = $(`.my_li[data-id='${user_id}']>.item_main`).attr("data-position").split(",");
        ...
        if(zoom >= 14){
            ...
            map.setZoomAndCenter(zoom, loc);
            console.log(`自定义标记点的点击事件的user_id=${user_id}, loc=${loc}`);
        }

    });

};
func_dict['custom_maker_double_click'] = custom_maker_double_click;  // 加入函数集
...
AMapUI.loadUI(['overlay/AwesomeMarker'], function (AwesomeMarker) {
    ...
    * */
    let get_awe_marker = function (arg_dict) {
        ...
        * </span>
        * */
        let user_id = typeof(arg_dict['_id']) === "undefined"? "guest" : arg_dict['_id'] ;
        ...
        custom_maker_hover(custom_marker);         // 添加鼠标悬浮事件
        custom_maker_double_click(custom_marker);  // 添加鼠标双击事件
...
check_map_init = function(func, arg_dict){
    ...
    $.post(server + "/manage/last_positions",function(json){
        ...
        if(l > 0){
            ...
            for(let i=0;i<l;i++){
                ...
                add_right_side_bar_item_func(data);
                /*不添加标注点的原因是标注点使用ws方式实时添加了*/
                // let get_awe_marker_func = func_dict['get_awe_marker'];
                // get_awe_marker_func(data);
            }
            $.getScript("../static/js/web_socket_stand_client.js");
        }else{}
    });
};
