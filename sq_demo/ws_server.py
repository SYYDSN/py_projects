...
from manage import sync_module


logger = log_module.get_logger("ws_server")
cache = mongo_db.cache
prev_gps_dict = sync_module.prev_gps_dict   # 存放上一次的实时gps信息
...
def send_all(message):  #
    handlers = WSIdMap.get_data()[0]
    user_id = message['user_id'].id
    employee = Employee.find_by_id(user_id)
    if isinstance(employee, Employee):
        employee = employee.to_flat_dict()
        employee['loc'] = message['loc']['coordinates']
        employee['raw_time'] = message['time'].strftime("%Y-%m-%d %H:%M:%S")
        employee['gps_time'] = mongo_db.get_datetime()
        prev_gps_dict[user_id] = employee  # 更新上一次的gps记录容器
        mes = {"mes_type": "real_time", "data": employee}
        [handler['handler'].write_message(mes) for handler in handlers.values()]
            ...
            if isinstance(message, dict):
                ...
                if user_id is None:
                    ...
                    if mes_type == "prev_gps":
                        ...
                        获取上一次的gps数据.
                        """
                        d = list(prev_gps_dict.values())
                        message = {"mes_type": "prev_gps", "data": d}  # 上一次的位置信息
                    else:
...
for u_id in sync_module.user_ids:
    sync_module.run_send_gps(u_id, send_all)
