...
from manage.sync_module import time_shard_report
...
#     """展示用户轨迹页面,旧，drop"""
#     if request.method.lower() == "get":
#         user_header_img = "../static/image/head_img/default_02.png"
#         phone_num = get_arg(request, "phone_num")
#         begin_date = get_arg(request, "begin_date")
#         end_date = get_arg(request, "end_date")
#         return render_template("manage/show_track.html",
        ...
        if user_id is None:
            ...
            if not isinstance(user_id, ObjectId):
                ...
                if archive is None:
                    ...
                    report = list()
                    try:
                        now_str = datetime.datetime.now().strftime("%F")
                        if date_str is None or date_str == now_str:
                            report = time_shard_report(user_id)
                        else:
