...
class Track(mongo_db.BaseDoc):
    ...
    @classmethod
    def create_item_from_gps(cls, gps_list: list) -> (dict, None):
        """
        批量创建一个用户的track信息。
        ...
        if length > 0:
            res = list()
            prev_item = gps_list[0]  # 上一个点，用来比较距离。
            for index, item in enumerate(gps_list):
                if index == 0 or index == length - 1:
                    [item.pop(p) for p in ["longitude", "latitude", 'altitude', '_id']]
                    res.append(item)
                else:
                    prev_pos = prev_item['loc']['coordinates']  # 上一个参考点的坐标
                    curr_pos = item['loc']['coordinates']  # 当前点的坐标
                    distance = position_distance(prev_pos, curr_pos)
                    try:
                        interval = (item['time'] - prev_item['time']).total_seconds()
                    except KeyError as e:
                        print(item)
                        print(prev_item)
                        raise  e
                    ...
                    print(item['loc'])
                    print(distance)
                    """位移大于5米或者大于5分钟，就要记录一下"""
                    if distance * 1000 >= 5 or interval > 60 * 5:
                        prev_item = item
                        [item.pop(p) for p in ["longitude", "latitude", 'altitude', '_id']]
                        res.append(item)
                    else:
            ...
            if len(inserted_results) > 0:
                ...
                last_position = cls.__get_last_position(doc_obj=last_inserted)
            else:
                last_position = None
    ...
    @classmethod
    def __get_last_position(cls, user_id: ObjectId = None, doc_obj: dict = None) -> dict:
        """
        从缓存中获取最近的位置信息（并更新缓存内容），返回最晚的位置信息的doc对象,
        这是一个可被复用的方法:
        * 直接被get_last_position调用。
        * 批量插入数据后调用。
        :param user_id: 用户id
        :param doc_obj: 已经保存到mongo数据库的，本批次写入的最后一个位置信息的doc对象。
        :return: 位置信息的doc对象
        """
        if user_id is None and doc_obj is None:
            raise ValueError("参数不能全为空")
        elif isinstance(user_id, ObjectId):
            o_id = user_id
        else:
            o_id = doc_obj['user_id'].id
        key = "last_position_{}".format(str(o_id))
        """从缓存中上一个最后一次的位置记录的doc"""
        prev_position = cache.get(key)
        reset_flag = False  # 是否在cache中重设prev_position的标志位
        if prev_position is None:
            """缓存中没有值，那就从数据库查"""
            user_dbref = DBRef(collection="user_info", database="platform_db", id=o_id)
            filter_dict = {"user_id": user_dbref}
            sort_dict = {"time": -1}  # -1表示倒序
            prev_position = Track.find_one_plus(filter_dict=filter_dict, sort_dict=sort_dict)
            if prev_position is not None:
                prev_position = Track.simple_doc(prev_position)
                user = User.find_by_id(o_id)
                phone_num = user.get_attr("phone_num")
                real_name = user.get_attr("real_name")
                prev_position['phone_num'] = phone_num
                if real_name is not None:
                    prev_position['real_name'] = real_name
                reset_flag = True
        else:
            pass
        if doc_obj is None:
            if prev_position is None:
                ...
                cache.set(key, prev_position, timeout=60 * 3)
                result = prev_position
        else:
            if prev_position is None:
                doc_obj = Track.simple_doc(doc_obj)  # 精简doc
                phone_num = User.find_by_id(o_id).get_attr("phone_num")
                doc_obj['phone_num'] = phone_num
                cache.set(key, doc_obj, timeout=60 * 3)
                result = doc_obj
            else:
                """比较哪个时间更晚"""
                prev_time = prev_position['time']
                prev_time = prev_time if isinstance(prev_time, datetime.datetime) else datetime.datetime.strptime(
                    prev_time, "%Y-%m-%d %H:%M:%S.%f")
                now_time = doc_obj['time']
                if (now_time - prev_time).total_seconds() > 0:
                    doc_obj = Track.simple_doc(doc_obj)  # 精简doc
                    phone_num = User.find_by_id(o_id).get_attr("phone_num")
                    doc_obj['phone_num'] = phone_num
                    cache.set(key, doc_obj, timeout=60 * 3)
    ...
    def get_last_position(cls, user_id: (ObjectId, list)) -> dict:
        ...
        for o_id in user_list:
            result = Track.__get_last_position(o_id)
...
if __name__ == "__main__":
    ...
    user = User.find_one_plus(filter_dict={"phone_num": "15618317376"})
    print(user)
    pass
