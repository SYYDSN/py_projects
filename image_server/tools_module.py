...
def save_platform_session(**kwargs):
    """保存平台操作者会话信息
...
def save_session(**kwargs):
    """保存操作者会话信息,和save_platform_session相比,这是一个拷贝
    :kwargs 必须包含 user_id user_name user_password三个参数
    return True代表保存成功，False代表保存失败
    """
    user_id = kwargs.get('user_id')
...
def clear_session():
    """清除平台操作者会话信息，注销使用。lear_platform_session的卡波被
    return None
    """
    """去掉session中的内容"""
    keys = list(session.keys())
...
def check_session(f):
    """检测管操作员是否登录的装饰器,check_platform_session的拷贝"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        user_name = session.get("user_name")  # 检测session中的user_name
        user_password = session.get("user_password")  # user_password
