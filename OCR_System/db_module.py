...
"""配置数据库相关参数"""
sql_host = "113.108.9.33"  # 生产用数据库地址
user = "root"  # 生产用数据库账户
password = "Java@701"  # 生产用数据库密码
database = "ocr_db"  # 数据库名称
charset = "utf8"  # 数据库字符集
pool_size = 50  # 数据库连接池
max_overflow = 300  # 数据库连接溢出数
...
def get_conn(host='127.0.0.1', a_user=user, a_password=password, a_database=database, a_charset=charset):
    ...
    while flag:
        try:
            conn = pymysql.connect(sql_host, a_user, a_password, a_database, charset=a_charset)
            flag = False
