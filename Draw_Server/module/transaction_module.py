...
class Transaction(mongo_db.BaseDoc):
    ...
    def last_ticket(cls, system_names: list) -> (None, dict):
        ...
        :return: {"domain1":last_tick1,"domain2":last_tick2}
        """
        res = dict()
        for name in system_names:
            filter_dict = {
                "$and": [
                    {"system": name},
                    {"$or": [
                        {"command": {"$in": ['buy', 'sell']}, "close_time": {"$exists": True, "$type": 9}},
                        {"command": {"$in": ['credit', 'balance']}, "time": {"$exists": True, "$type": 9}},
                    ]},
                    {"upload": 1}
                ]
            ...
            holding = cls.get_holdings(domain=name)
            if len(holding) == 0:
                last_holding = -1
            else:
                last_holding = holding[-1]['ticket']
            record = record if record <= last_holding else last_holding
            res[name] = {"last_ticket": record, "holding": holding}
        return res

    @classmethod
    def get_holdings(cls, domain: str) -> list:
        """
        获取数据库中，还在持仓的记录
        :return:
        """
        now = datetime.datetime.now()
        limit = now - datetime.timedelta(days=60)
        filter_dict = {
            "system": domain,
            "command": {"$in": ['buy', 'sell']},
            "$or":
        ...
        sort_dict = {"ticket": -1}
        p = ['system', 'ticket', "open_time"]
        holdings = cls.find_plus(filter_dict=filter_dict, sort_dict=sort_dict, projection=p, to_dict=True)
        return holdings
...
if __name__ == "__main__":
    print(Transaction.last_ticket(['office.shengfxchina.com:8443', 'office.shengfx888.com']))
    for x in ['office.shengfxchina.com:8443', 'office.shengfx888.com']:
        print(Transaction.get_holdings(x))
    pass